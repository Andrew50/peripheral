...
- `apply-fixes.sh`: Script to fix deployment issues and ensure proper configuration (now located in prod/scripts/)
...
If you encounter issues with deployments, you can use the `prod/scripts/apply-fixes.sh` script to fix common problems:

```bash
# Apply fixes using prod branch images (default)
prod/scripts/apply-fixes.sh

# Or specify a branch to use those images
prod/scripts/apply-fixes.sh prod  # Use production images
prod/scripts/apply-fixes.sh dev   # Use development images

# You can also override the Docker username
export DOCKER_USERNAME=your_docker_username
prod/scripts/apply-fixes.sh dev
