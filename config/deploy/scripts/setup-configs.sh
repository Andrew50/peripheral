...
: "${FRED_API_KEY:?Missing FRED_API_KEY}"
...
: "${TELEGRAM_USER_NOTIFICATION_BOT_TOKEN:?Missing TELEGRAM_USER_NOTIFICATION_BOT_TOKEN}"
: "${TELEGRAM_BEN_TWEETS_BOT_TOKEN:?Missing TELEGRAM_BEN_TWEETS_BOT_TOKEN}"
: "${SMTP_FROM_EMAIL:?Missing SMTP_FROM_EMAIL}"
: "${SMTP_PASSWORD:?Missing SMTP_PASSWORD}"
: "${SMTP_HOST:?Missing SMTP_HOST}"
: "${SMTP_OTP_PORT:?Missing SMTP_OTP_PORT}"
: "${EMAIL_FROM_ADDRESS:?Missing EMAIL_FROM_ADDRESS}"
...
FRED_B64=$(echo -n "$FRED_API_KEY" | base64 -w 0)
...
TELEGRAM_USER_NOTIFICATION_BOT_TOKEN_B64=$(echo -n "$TELEGRAM_USER_NOTIFICATION_BOT_TOKEN" | base64 -w 0)
TELEGRAM_BEN_TWEETS_BOT_TOKEN_B64=$(echo -n "$TELEGRAM_BEN_TWEETS_BOT_TOKEN" | base64 -w 0)
SMTP_FROM_EMAIL_B64=$(echo -n "$SMTP_FROM_EMAIL" | base64 -w 0)
SMTP_PASSWORD_B64=$(echo -n "$SMTP_PASSWORD" | base64 -w 0)
SMTP_HOST_B64=$(echo -n "$SMTP_HOST" | base64 -w 0)
SMTP_OTP_PORT_B64=$(echo -n "$SMTP_OTP_PORT" | base64 -w 0)
EMAIL_FROM_ADDRESS_B64=$(echo -n "$EMAIL_FROM_ADDRESS" | base64 -w 0)

# Telegram secrets are already base64 encoded from GitHub secrets
# No need to encode them again

export DB_B64 REDIS_B64 POLYGON_B64 POLYGON_S3_KEY_B64 POLYGON_S3_SECRET_B64 GEMINI_B64 GEMINI_API_B64 TWITTER_API_IO_KEY_B64 OPENAI_B64 FRED_B64 GROK_B64 X_API_KEY_B64 X_API_SECRET_B64 X_ACCESS_TOKEN_B64 X_ACCESS_SECRET_B64 GOOGLE_ID_B64 GOOGLE_SECRET_B64 JWT_B64 CLOUDFLARE_TOKEN_B64 STRIPE_SECRET_B64 STRIPE_WEBHOOK_B64 STRIPE_PUBLISHABLE_B64 TELEGRAM_BOT_TOKEN_B64 TELEGRAM_CHAT_ID_B64 TELEGRAM_USER_NOTIFICATION_BOT_TOKEN_B64 TELEGRAM_BEN_TWEETS_BOT_TOKEN_B64 SMTP_FROM_EMAIL_B64 SMTP_PASSWORD_B64 SMTP_HOST_B64 SMTP_OTP_PORT_B64 EMAIL_FROM_ADDRESS_B64
