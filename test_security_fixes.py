...
''',
        ...
        for test_case in test_cases:
            try:
                ...
                if test_case['should_fail']:
                    print(f"  ❌ {test_case['name']}: Should have been blocked but executed")
                else:
                ...
                if test_case['should_fail']:
                    print(f"  ✓ {test_case['name']}: Properly blocked with security error")
                    passed += 1
                else:
                    print(f"  ❌ {test_case['name']}: Safe code was incorrectly blocked")
                    
            except Exception as e:
                if test_case['should_fail']:
                    print(f"  ✓ {test_case['name']}: Blocked with error: {type(e).__name__}")
                    passed += 1
                else:
    ...
    async def test_input_validation(self):
        ...
        try:
            ...
            data = await self.data_provider.scan_universe(sort_by="'; DROP TABLE securities; --")
...
'''
        ...
        try:
            ...
            assert 'trading_strategy' in result, "Strategy should produce trading_strategy result"
            strategy_result = result['trading_strategy']
            assert 'signal' in strategy_result, "Strategy should produce trading signal"
            assert 'current_price' in strategy_result, "Strategy should have current price"
            assert strategy_result['strategy_executed'] == True, "Strategy should execute successfully"
