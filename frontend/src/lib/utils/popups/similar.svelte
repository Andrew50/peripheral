...
<script lang="ts" context="module">
	...
	export function querySimilarInstances(event: MouseEvent, baseIns: Instance): void {
		privateRequest<Instance[]>('getSimilarInstances', {
			ticker: baseIns.ticker,
			securityId: baseIns.securityId,
			timeframe: baseIns.timeframe,
			timestamp: baseIns.timestamp
		}).then((v: Instance[]) => {
...
{#if $similarQuery.status === 'active'}
	<div
		...
		bind:this={menu}
		style="top: {$similarQuery.y}px; left: {$similarQuery.x}px;"
	>
				...
				<table>
					...
					<tbody>
						{#each $similarQuery.similarInstances as instance}
							<tr class="defalt-tr">
								<td
									on:click={() => queryChart(instance, true)}
									on:contextmenu={(e) => {
										e.preventDefault();
										closeMenu();
										queryInstanceRightClick(e, instance, 'similar');
									}}>{instance.ticker}</td
								>
							</tr>
						{/each}
