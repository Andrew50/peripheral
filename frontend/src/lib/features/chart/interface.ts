...
export type ChartEvent = "" | "replay" | "realtime"

export let selectedChartId: ChartId = 0

export function setActiveChart(chartId:number){selectedChartId = chartId}

export let chartQueryDispatcher: Writable<ChartQueryDispatch> = writable({timestamp:0, extendedHours:false, timeframe:"1d",ticker:""})
export let chartEventDispatcher: Writable<ChartEventDispatch> = writable({timestamp:0, extendedHours:false, timeframe:"1d",ticker:""})
export function eventChart(event:ChartEvent,chartId:ChartId="all"){
    chartEventDispatcher.set({event:event,chartId:chartId})
}
