<script lang="ts">
	...
	async function retryMessage(message: Message) {
		...
		if (message.sender === 'assistant') {
			...
			userMessage = $messagesStore.find(
				(msg) => msg.message_id === userMessageId && msg.sender === 'user'
			);
		...
		try {
			...
			if (response && (response as any).success) {
				...
				const userMessageIndex = $messagesStore.findIndex(
					(msg) => msg.message_id === backendMessageId
				);
