<script lang="ts">
    ...
    import { getNotes, createNote, deleteNote, toggleNotePin, toggleNoteArchive } from '$lib/services/notes';
    ...
    async function handleTogglePin(note: Note) {
        try {
            await toggleNotePin(note.noteId, !note.isPinned);
            await loadNotes();
        } catch (err) {
    ...
    {#if showNewNoteForm}
        <div class="new-note-form">
            ...
            <div class="form-actions">
                <button type="button" class="btn-secondary" on:click={() => showNewNoteForm = false}>Cancel</button>
                ...
                <div class="note-card {note.isPinned ? 'pinned' : ''}">
                    <div class="note-header">
                        ...
                        <div class="note-actions">
                            <button class="icon-btn" on:click={() => handleTogglePin(note)} title={note.isPinned ? 'Unpin' : 'Pin'}>
                                {note.isPinned ? '📌' : '📍'}
                            </button>
                            <button class="icon-btn" on:click={() => handleToggleArchive(note)} title={note.isArchived ? 'Unarchive' : 'Archive'}>
                                {note.isArchived ? '🔄' : '📁'}
                            </button>
                            <button class="icon-btn" on:click={() => handleEditNote(note.noteId)} title="Edit">
                                ✏️
                            </button>
                            <button class="icon-btn" on:click={() => handleDeleteNote(note.noteId)} title="Delete">
                                🗑️
                            </button>
                        </div>
                    </div>
