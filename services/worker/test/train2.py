...
def train_gan_with_reconstruction_loss(
    gan, generator, discriminator, data, epochs=100, initial_lambda_reconstruction=0.5
):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            print(
                f"Epoch: {epoch}, Discriminator Loss: {d_loss_real[0]}, Generator Loss: {g_loss}, Reconstruction Loss: {recon_loss}"
            )

            # Reduce the influence of reconstruction loss over time if the GAN is performing well
            if d_loss_fake_value < 0.5:
                lambda_reconstruction = max(
                    0.1, lambda_reconstruction * 0.9
                )  # Decrease, but keep it above 0.1
...
def iterative_training_loop_with_classifier_guidance(
    data, generator, classifier, gan, discriminator, iterations=100
):
    ...
    for i in range(iterations):
        ...
        if len(labeled_data) >= 10:  # Retrain in batches
            ...
            charts = np.array(
                charts
            )  # .reshape(-1, 28, 28, 1)  # Reshape to fit model input
            ...
            if positive_examples:
                ...
                train_gan_with_reconstruction_loss(
                    gan, generator, discriminator, positive_examples, epochs=10
                )
            labeled_data.clear()
        print(
...
def build_generator():
    ...
    model.add(layers.LeakyReLU(0.2))
    model.add(
        layers.Dense(30 * 4, activation="tanh")
    )  # Assuming output shape for synthetic charts
...
def build_discriminator():
    model = models.Sequential()
    model.add(
        layers.Conv1D(64, kernel_size=3, activation="relu", input_shape=(30, 4))
    )  # Assuming sequence length of 30
...
def build_gan(generator, discriminator):
    discriminator.trainable = (
        False  # Freeze discriminator's weights during generator training
    )
...
train_gan_with_reconstruction_loss(
    gan, generator, discriminator, initial_data, epochs=10
)
iterative_training_loop_with_classifier_guidance(
    data, generator, classifier, gan, discriminator, iterations=100000
)
