...
def get_sector_info(ticker_symbol):
    try:
        ...
        print(
            f"{get_timestamp()} - Error fetching info for {ticker_symbol}: {str(e)}",
            flush=True,
        )
        return {"ticker": ticker_symbol, "sector": "Unknown", "industry": "Unknown"}
...
def update_sectors(conn):
    ...
    try:
        with conn.db.cursor() as cursor:
            ...
            for ticker in tickers:
                ...
                if USE_DATABASE:
                    try:
                        ...
                        conn.db.rollback()
                        print(
                            f"{get_timestamp()} - Error updating {ticker}: {str(e)}",
                            flush=True,
                        )
                        continue
            ...
            if USE_DATABASE:
                print(
                    f"{get_timestamp()} - Completed! Successfully updated {total_updates} securities",
                    flush=True,
                )
