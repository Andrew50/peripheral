...
CREATE TABLE IF NOT EXISTS python_strategies (
    ...
    -- Array of required Python libraries
    data_prep_sql TEXT,
    -- Optional SQL query for data preparation
    execution_mode VARCHAR(20) DEFAULT 'python' CHECK (
...
CREATE TABLE IF NOT EXISTS python_executions (
    ...
    -- K8s node/pod that executed the strategy
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_python_strategies_active ON python_strategies(is_active)
...
CREATE INDEX IF NOT EXISTS idx_python_environments_active ON python_environments(is_active)
WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_python_environments_default ON python_environments(is_default)
...
CREATE OR REPLACE FUNCTION update_python_strategies_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION update_python_environments_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_python_strategies_updated_at BEFORE
UPDATE ON python_strategies FOR EACH ROW EXECUTE FUNCTION update_python_strategies_updated_at();
CREATE TRIGGER trigger_python_environments_updated_at BEFORE
UPDATE ON python_environments FOR EACH ROW EXECUTE FUNCTION update_python_environments_updated_at();
...
SELECT ps.*,
    ...
    COUNT(
        CASE
            WHEN pe.status = 'completed' THEN 1
        END
    ) as successful_executions,
    COUNT(
        CASE
            WHEN pe.status = 'failed' THEN 1
        END
    ) as failed_executions,
    AVG(
        CASE
            WHEN pe.status = 'completed' THEN pe.execution_time_ms
        END
...
VALUES (
        21,
        'Add why_is_it_moving table for tracking stock movement explanations'
    );
COMMIT;