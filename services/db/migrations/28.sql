...
CREATE INDEX IF NOT EXISTS idx_conversation_messages_archive_reason ON conversation_messages(archive_reason)
WHERE archive_reason IS NOT NULL;
-- Update schema version
INSERT INTO schema_versions (version, description)
VALUES (
        28,
        'Added archive_reason field to conversation_messages table'
...
WHERE NOT EXISTS (
        SELECT 1
        FROM strategies s
        WHERE s.strategyId = ps.strategy_id
    );
...
CREATE OR REPLACE FUNCTION update_strategies_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_strategies_updated_at BEFORE
UPDATE ON strategies FOR EACH ROW EXECUTE FUNCTION update_strategies_updated_at();
