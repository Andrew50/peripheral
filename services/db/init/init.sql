...
CREATE INDEX idxStudiesByTagUserId on studies(
    userId,
    securityId,
    strategyId,
    timestamp,
...
INSERT
    OR
UPDATE
    OR DELETE ON conversation_messages FOR EACH ROW EXECUTE FUNCTION update_conversation_updated_at();
-- Function to auto-increment message_order
CREATE OR REPLACE FUNCTION set_message_order() RETURNS TRIGGER AS $$ BEGIN IF NEW.message_order IS NULL THEN
SELECT COALESCE(MAX(message_order), 0) + 1 INTO NEW.message_order
...
CREATE TRIGGER trigger_set_message_order BEFORE
INSERT ON conversation_messages FOR EACH ROW EXECUTE FUNCTION set_message_order();
-- Function to update conversation stats (token count, message count)
CREATE OR REPLACE FUNCTION update_conversation_stats() RETURNS TRIGGER AS $$ BEGIN -- Update conversation statistics 
UPDATE conversations
