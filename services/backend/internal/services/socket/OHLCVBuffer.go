...
func (b *OHLCVBuffer) addBar(timestamp int64, securityID int, bar models.EquityAgg) {
	...
	} else if timestamp < b.currentTimestamp {
	...
	if isLateArrival {
		log.Printf("📊 Late arrival detected: security %d, timestamp %d (current: %d)",
			record.SecurityID, record.Timestamp, b.currentTimestamp)
	}
}
