...
func UpdateDailyOHLCV(conn *utils.Conn) error {
	start := time.Now()
	defer func() {
	...
	maxConcurrency := 2 // Reduce from 5 to 2
	...
	for _, date := range dates {
		...
		go func(date time.Time) {
			...
			ohlcvResponse, err := utils.GetDailyOHLCV(conn.Polygon, dateStr, ctx)
			if err != nil {
...
func storeDailyOHLCVParallel(conn *utils.Conn, ohlcvResponse *models.GetGroupedDailyAggsResponse, date time.Time, securityCache *sync.Map) error {
	...
	var wg sync.WaitGroup
	maxConcurrency := 2 // Reduce from 3 to 2
	sem := semaphore.NewWeighted(int64(maxConcurrency))
	...
	for i := 0; i < len(results); i += batchSize {
		...
		go func(startIdx int) {
			...
			for _, record := range currentBatch {
				...
				if !exists {
					var err error
					securityID, err = utils.GetSecurityID(conn, ticker, date)
					if err != nil {
...
func batchPreloadTickers(conn *utils.Conn, tickers map[string]bool, date time.Time, securityCache *sync.Map) {
