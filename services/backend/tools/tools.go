...
func initTools() {
	...
	Tools = map[string]Tool{
		...
		"getPrevClose": {
			FunctionDeclaration: &genai.FunctionDeclaration{
				Name:        "getPrevClose",
				Description: "Retrieves the previous closing price for a security. If we are past market close, it will return today's close.",
				Parameters: &genai.Schema{
					Type: genai.TypeObject,
					Properties: map[string]*genai.Schema{
						"ticker": {
...
func DeleteAccount(conn *utils.Conn, userID int, rawArgs json.RawMessage) (interface{}, error) {
	...
	// Delete strategys
	_, err = tx.Exec(ctx, "DELETE FROM strategys WHERE userId = $1", userID)
	if err != nil {
		fmt.Printf("ERROR: Failed to delete strategys: %v\n", err)
		// Continue despite error
	}
