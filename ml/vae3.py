...
def build_unconditional_encoder():
    ...
    for units, kernel_size in zip(CONV_FILTER_UNITS, CONV_FILTER_KERNAL_SIZES):
        x = Conv1D(filters=units, kernel_size=kernel_size, activation='tanh', padding='same')(x)
...
def build_unconditional_decoder():
    ...
    for units in reversed(BI_LSTM_UNITS):
        x = Bidirectional(LSTM(units=units, return_sequences=True))(x)
    for units, kernel_size in zip(reversed(CONV_FILTER_UNITS), reversed(CONV_FILTER_KERNAL_SIZES)):
        x = Conv1D(filters=units, kernel_size=kernel_size, activation='tanh', padding='same')(x)
...
def iterative_training_loop_with_classifier_guidance(decoder, classifier, vae, iterations=100):
...
random_stock_data = np.array([augment_data(normalized_initial_data) for _ in range(1000)])
...
trainingSample, validationSample = getSample(conn,setupID,interval,.08, .8, .98)
#print(trainingSample)
xTrainingData, yTrainingData = getTensor(conn,trainingSample,interval,bars,normalize="min-max")
...
# For encoder
unconditional_encoder_layers = {layer.name: layer for layer in unconditional_encoder.layers}
...
for layer_name in unconditional_encoder_layers:
    if layer_name in conditional_encoder_layers:
        ...
        if 'labels' not in layer_name and 'embedding' not in layer_name and layer_uncond.get_weights():
            try:
...
# For decoder
unconditional_decoder_layers = {layer.name: layer for layer in unconditional_decoder.layers}
...
iterative_training_loop_with_classifier_guidance(decoder, classifier, vae, iterations=100000)
