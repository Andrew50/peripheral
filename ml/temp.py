    ...
    for i in range(len(trade_df)):
        ...
        if trade['Trade Direction'] == "Long":
            if total_exit_shares > 0:  # Only calculate if we have exits
                pnl = round(total_exit_value - (total_entry_value * (total_exit_shares / total_entry_shares)), 2)
        else:  # Short
            if total_exit_shares > 0:  # Only calculate if we have exits
...
def process_data(cv):
    ...
    columns = ['Ticker', 'Trade Direction', 'Date', 'Status', 'Open Quantity', 'Closed P/L']
...
def update_trade(trade_df, trade, open_trade_row):
    ...
    if trade_direction == "Short": 
        if 'Short' in trade['Trade Description'] or 'Sell to Open' in trade['Trade Description']:
            new_trade_direction = "Entry"
        else: 
            new_trade_direction = "Exit"
    elif trade_direction == "Long": 
        if 'Buy' in trade['Trade Description'] or 'Buy to Open' in trade['Trade Description']:
            new_trade_direction = "Entry"
        else: 
            new_trade_direction = "Exit"
    ...
    if new_trade_direction == "Entry":
            ...
            if pd.isna(open_trade_row[f'Entry Time {i}']):
                ...
                if 'Limit' in trade['Trade Description']:
                    if 'FILLED AT' in trade['Status']:
                        trade_df.at[open_trade_row.name, f'Entry Price {i}'] = trade['Status'].split('$')[1]
                        shares = float(trade['Quantity'].replace(',', ''))
                    elif 'PARTIAL' in trade['Status']:
                        status_parts = trade['Status'].split('\n')
                        if len(status_parts) >= 4:  # Assuming partial fill info is on the 4th line
                            shares = float(status_parts[3].split(' ')[0].replace(',', ''))
                            trade_df.at[open_trade_row.name, f'Entry Price {i}'] = trade['Trade Description'].split('$')[1]
                elif 'Market' in trade['Trade Description']:
                    trade_df.at[open_trade_row.name, f'Entry Price {i}'] = trade['Status'].split('$')[1]
                    shares = float(trade['Quantity'].replace(',', ''))
                trade_df.at[open_trade_row.name, f'Entry Shares {i}'] = shares 
                if trade_direction == "Short": 
                    trade_df.at[open_trade_row.name, f'Open Quantity'] = float(trade_df.at[open_trade_row.name, f'Open Quantity'])-shares
                elif trade_direction == "Long": 
                    trade_df.at[open_trade_row.name, f'Open Quantity'] = float(trade_df.at[open_trade_row.name, f'Open Quantity'])+shares
                break 
        ...
        for i in range(1, 51):
            if pd.isna(open_trade_row[f'Exit Time {i}']):
                ...
                shares = 0 
                if 'Limit' in trade['Trade Description'] or 'Stop' in trade['Trade Description']:
                    if 'FILLED AT' in trade['Status']:
                        trade_df.at[open_trade_row.name, f'Exit Price {i}'] = trade['Status'].split('$')[1]
                        shares= float(trade['Quantity'].replace(',', ''))
                    elif 'PARTIAL' in trade['Status']:
                        status_parts = trade['Status'].split('\n')
                        if len(status_parts) >= 4:  # Assuming partial fill info is on the 4th line
                            shares = float(status_parts[3].split(' ')[0].replace(',', ''))
                            trade_df.at[open_trade_row.name, f'Exit Price {i}'] = trade['Trade Description'].split('$')[1]
                elif 'Market' in trade['Trade Description']:
                    trade_df.at[open_trade_row.name, f'Exit Price {i}'] = trade['Status'].split('$')[1]
                    shares = float(trade['Quantity'].replace(',', ''))
...
def create_trade(trade_df, trade):
    ...
    if 'Limit' in trade_description:
        if 'FILLED AT' in fidelity_trade_status:
            ...
            shares = float(fidelity_trade_status.split('\n')[3].split(' ')[0].replace(',', ''))
