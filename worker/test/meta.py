...
def create_vae_and_decoder(input_shape=(30, 4), latent_dim=2):
    ...
    x_decoded = layers.Conv1D(4, kernel_size=3, activation='tanh', padding='same')(h_decoded)
...
def createModel():
    ...
    for units, kernel_size in zip(CONV_FILTER_UNITS, CONV_FILTER_KERNAL_SIZES):
        model.add(layers.Conv1D(filters=units, kernel_size=kernel_size, activation='relu'))
    ...
    model.add(layers.Bidirectional(layers.LSTM(units=BI_LSTM_UNITS[-1], return_sequences=False)))
...
def adaptive_exploration(initial_data, vae, encoder, decoder, classifier, iterations=100):
    ...
    for i in range(iterations):
        ...
        perturbations = np.random.normal(0, current_noise_level, (10, LATENT_DIM))  # Generate 10 variations
        ...
        uncertain_charts = select_uncertain_charts(classifier, generated_samples, max_samples=5)
        ...
        print(f"Iteration {i + 1}: Success Rate = {success_rate:.2f}, Noise Level = {current_noise_level:.4f}")
...
vae, encoder, decoder = create_vae_and_decoder()  # Define this function to create the VAE and decoder
