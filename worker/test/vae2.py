...
from tensorflow.keras.layers import Dense, LSTM, Bidirectional, Dropout, Conv1D, Flatten, Lambda, Input, Reshape
...
def build_encoder():
    ...
    label_embedding = layers.Embedding(input_dim=2, output_dim=4)(labels)  # Assuming binary labels (0, 1)
    label_embedding = layers.Flatten()(label_embedding)
    label_embedding = layers.RepeatVector(30)(label_embedding)  # Repeat to match input sequence length
    
    x = layers.Concatenate()([inputs, label_embedding])  # Concatenate label with input data
    for units, kernal_size in zip(CONV_FILTER_UNITS, CONV_FILTER_KERNAL_SIZES):
        x = Conv1D(filters=units, kernel_size=kernal_size, activation='tanh', padding='same')(x)
    ...
    encoder = Model([inputs, labels], [z_mean, z_log_var, z], name='encoder')  # Note: now takes labels as input
    return encoder
...
def build_decoder():
    ...
    for units in reversed(BI_LSTM_UNITS):
        x = Bidirectional(LSTM(units=units, return_sequences=True))(x)
    for units, kernal_size in zip(reversed(CONV_FILTER_UNITS), reversed(CONV_FILTER_KERNAL_SIZES)):
        x = Conv1D(filters=units, kernel_size=kernal_size, activation='tanh', padding='same')(x)
    outputs = Conv1D(filters=4, kernel_size=1, activation='tanh', padding='same')(x)
    decoder = Model([latent_inputs, labels], outputs, name='decoder')  # Note: now takes labels as input
    return decoder
...
class VAE(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            sample_weights = tf.where(tf.equal(labels, 1), 2.0, 1.0)  # Weight 'yes' samples more
            sample_weights = tf.cast(sample_weights, tf.float32)
            
            # Compute reconstruction loss with sample weights
            reconstruction_loss = tf.reduce_mean(
                sample_weights * tf.reduce_sum(tf.square(targets - reconstruction), axis=[1, 2])
            )
...
def iterative_training_loop_with_classifier_guidance(data, decoder, classifier, vae, iterations=100):
...
training_data = np.array([data for data, label in labeled_data])  # Combine all input data, retaining the batch dimension
labels = np.array([label for _, label in labeled_data])  # Store labels as a numpy array

# Instantiate the models
# Prepare training data including both positive and negative examples
training_data = np.array([data for data, label in labeled_data])  # Combine all input data, retaining the batch dimension
training_data = np.squeeze(training_data)  # Remove any extra dimensions to get (batch_size, 30, 4)
...
print(f"Training data shape: {training_data.shape}")  # Should print: (number_of_samples, 30, 4)
...
iterative_training_loop_with_classifier_guidance(data, decoder, classifier, vae, iterations=100000)
