...
def human_in_the_loop_training(initial_data, vae, encoder, decoder, iterations=100):
    labeled_data = [(initial_data, 1)]  # Start with the initial sample labeled as a pattern

    for i in range(iterations):
        # Prepare data for VAE training
        train_data = np.array([x for x, _ in labeled_data])
        train_data = train_data.reshape(-1, initial_data.shape[0], 4)  # Reshape to fit VAE input
        ...
        z_mean = encoder.predict(train_data)  # Use the encoder to get the latent representation
        ...
        print(f"Iteration {i + 1}: Sample labeled as {'pattern' if user_labeling == 1 else 'not a pattern'}")
