(Note: This project uses an AI "agent" internally for strategy generation and user assistance (chatbot). While it's not a user-facing agent in terms of separate automation, this guide will clarify how the agent system works and how to extend or maintain it.)
Overview of the Agent System
The Atlantis Agent is an AI-driven component that helps users in two ways:
...
Chat Assistant: Answering user questions about markets or the platform, and guiding them in using Atlantis (like a built-in support/chatbot).
...
This likely includes instructions like "You are Atlantis AI assistant. You have knowledge of trading and can access user’s strategies. Do not provide financial advice, just informational...".
...
Automation Recipes
The agent currently is user-facing (reactive to prompts), but we might have internal automation using it:
...
Evaluate the content for accuracy and appropriateness. Since it's internal, we don't have a public-facing toxicity concern, but we do care about correctness of strategy code and not giving unbounded financial advice (disclaimers might be needed).
Agent Configuration
...
This guide ensures developers understand the interplay between our AI agent and the rest of the system, and how to maintain and extend it responsibly. Always test agent changes thoroughly, as they can affect user trust (we promise certain capabilities and must deliver correct output, especially for code generation where mistakes could cost money if a strategy behaves incorrectly). Keep the agent helpful, accurate, and aligned with our platform's purpose – empowering traders with AI, not misleading them.
...
The Atlantis platform needs to store large volumes of financial data (e.g., historical price data, fundamental metrics) as well as relational data (user accounts, strategies, etc.). We considered using a NoSQL or specialized time-series database to handle the time-series data (like InfluxDB or MongoDB for flexibility) separate from relational data. Decision:
